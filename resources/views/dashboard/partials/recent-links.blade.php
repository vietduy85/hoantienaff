<div>
    <div class="flex items-center gap-2 max-[390px]:mb-2 mb-3">
        ...
        <h3 class="text-sm font-semibold text-gray-700">Link Hoàn Tiền Gần Đây</h3>
    </div>
                        ...
                        @if ($link->product_name)
                            @if ($link->affiliate_url)
                                <a href="{{ $link->affiliate_url }}"
                                   ...
                                   class="text-emerald-600 hover:text-emerald-700 hover:underline cursor-pointer truncate whitespace-nowrap overflow-hidden text-ellipsis text-sm block">
                                    {{ $link->product_name }}
                                </a>
                            @else
                                <span class="text-gray-400 truncate whitespace-nowrap overflow-hidden text-ellipsis text-sm block">
                                    {{ $link->product_name }}
                                </span>
                            @endif
                        @endif
                    ...
                    <div class="flex items-center gap-1 shrink-0">
                        @if ($link->affiliate_url)
                            <a href="{{ $link->affiliate_url }}"
                               target="_blank"
                               rel="noopener noreferrer"
                               class="text-xs px-3 h-8 flex items-center rounded-lg bg-emerald-500 hover:bg-emerald-600 active:bg-emerald-700 text-white transition-colors font-medium whitespace-nowrap">
                                Mua ngay
                            </a>
                        @endif
                        @if (!$link->is_pinned)
                            <form action="{{ route('link-requests.toggle-pin', $link) }}" method="POST">
                                @csrf
                                <button type="submit"
                                        class="text-xs px-2 h-8 flex items-center gap-1 rounded-lg bg-gray-50 hover:bg-amber-50 active:bg-amber-100 text-gray-400 hover:text-amber-500 transition-colors whitespace-nowrap"
                                        title="Ghim link">
