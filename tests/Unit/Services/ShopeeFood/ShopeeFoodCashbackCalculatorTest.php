...
class ShopeeFoodCashbackCalculatorTest extends TestCase
{
    ...
    {
        // 20 / 100 = 0.20 -> >= 0.12 -> 60%
        $r = $this->calc->calculate(20.0, 100.0, false);
    ...
    {
        // 5 / 100 = 0.05 -> 50%
        $r = $this->calc->calculate(5.0, 100.0, false);

        $this->assertSame(0.50, $r['cashback_rate']);
        $this->assertSame(2.0, $r['cashback_amount']);
    }

    public function test_pending_shows_estimate_not_zero(): void
    {
        // Pending (not cancelled, not completed) still shows an estimate:
        // 450/5000 = 0.09 -> tier 50% -> floor(450*0.5) = 225.
        $r = $this->calc->calculate(450.0, 5000.0, false);

        $this->assertSame(0.50, $r['cashback_rate']);
        $this->assertSame(225.0, $r['cashback_amount']);
    }

    public function test_cancelled_produces_zero_amount(): void
    {
    ...
    public function test_zero_commission_produces_zero_amount(): void
    {
        $r = $this->calc->calculate(0.0, 100.0, false);
    ...
    public function test_amount_is_floored(): void
    {
        ...
        $r = $this->calc->calculate(75.0, 100.0, false);
